...
export const Basket = () => {
  ...
  return (
    <div className={classes.container}>
      <div className={classes.buttonMakeOrder}>{!!localStorageBasket?.length && <ButtonMakeOrder />}</div>
      <Modal show={show} onHide={handleClose}>
        <Modal.Header closeButton>
          <Modal.Title>Remove phone</Modal.Title>
        </Modal.Header>
        ...
        localStorageBasket.map((item) => (
          <div className={classes.phoneCardBasket} key={item.id}>
            ...
            <div className={classes.counterAndDelete}>
              ...
              <Counter idphone={item.id} localStorageBasket={localStorageBasket} setLocalStorageBasket={setLocalStorageBasket} />
            </div>
          </div>
        ))
      ) : (
